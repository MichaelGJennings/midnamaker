...
export class ManufacturerManager {
    ...
    async createAndLoadMidnam(manufacturer, model, deviceId) {
        try {
            ...
            if (isHostedVersion()) {
                // Create MIDNAM in browser storage for hosted version
...
<MIDINameDocument>
  ...
  <MasterDeviceNames>
    ...
    <Model>${Utils.escapeXml(model)}</Model>
    ...
    <NoteNameList Name="Notes">
      ...
      <Note Number="42" Name="Hi-Hat" />
                ...
                filePath = `patchfiles/${manufacturer}_${model}.midnam`;
                ...
                // Parse the XML to create deviceData structure
                deviceData = await this.parseDeviceXML(midnamXml, deviceId, manufacturer, model, filePath);

                Utils.showNotification(`Created ${model} in browser storage`, 'success');
            } else {
                // Call the API to create the midnam file for local version
                ...
                filePath = result.midnam_path;
            }

            // Ensure deviceData has file_path
            if (!deviceData.file_path) {
                deviceData.file_path = filePath;
            }
            ...
            if (isHostedVersion()) {
                await this.refreshManufacturerListDynamic();
            } else {
